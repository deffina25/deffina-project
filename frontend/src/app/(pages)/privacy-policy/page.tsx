...
const Page = async () => {
  try {
    // Используем fallback для API URL
    const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:1337';

    // Дополнительная проверка на строку "undefined"
    if (!apiUrl || apiUrl === 'undefined') {
      console.error('❌ API URL невалидный:', apiUrl);
      return <ErrorPage />;
    }

    console.log(
      '🔍 Загружаем политику конфиденциальности с:',
    ...
    if (!data?.title || !data?.text) {
      console.warn('⚠️ Неполные данные в ответе:', data);
    }
      ...
      <div className="container mx-auto mt-[172px] max-w-[1202px] px-4 text-white">
        <div className="border-1 border-[#8F8D81] pb-[72px]">
          <div className="relative border-b border-[#8F8D81] px-4 py-[49px] text-center text-[28px] leading-0 font-bold md:px-[32px] md:py-[73px] md:text-[56px] lg:text-[80px]">
            ...
            />
            <span>{data?.title || 'Политика конфиденциальности'}</span>
            <Image
              src={PlusTitleIcon}
              alt="Плюс"
              width={24}
